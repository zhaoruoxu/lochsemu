...
struct X86SIMD {
    ...
    INLINE const u64 &MMReg(u32 regNum) const;
    INLINE u128 &   XMMReg(u32 regNum);
    INLINE const u128 &XMMReg(u32 regNum) const;
};
...
INLINE const u64 & X86SIMD::MMReg( u32 regNum ) const
{
    switch (regNum) {
    case REG0: { return MM[0]; } break;
    case REG1: { return MM[1]; } break;
...
INLINE const u128 & X86SIMD::XMMReg( u32 regNum ) const
{
    switch (regNum) {
    case REG0: { return XMM[0]; } break;
    case REG1: { return XMM[1]; } break;
